...
class Agent:
    ...
    def update_in(self, walls, trees, xagents):
        ...
        # save data
        self.data.save_sensors(self.sensors.vs_sensors, self.sensors.olf_sensors, vs_info, olf_info, e_info)
        self.data.save_nnet(self.net.net_state, self.net.net_out)
        # communication
        if self.com_n > 0:
            self.axcom.update_out(com)
            self.data.save_com(self.axcom.com_area, com_info, self.axcom.com_out)
    ...
    def update_location(self, bounds, walls, trees, xagents, crush_damage):
        # call location fxs
        self.define_body_area()
        self.check_overlap(bounds, walls, trees, xagents, crush_damage)
        self.define_feeding_area()
